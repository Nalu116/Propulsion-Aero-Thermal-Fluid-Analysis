The scripts found in this repo are various levels of fidelity for aero-thermal-fluid analysis (specifically with propulsive use cases in mind, though any thermo-fluid use case that utilized quasi-1D flow and area/mach relations could find this useful). Examples include using the negative shaft work adiabatic flow for a first order approximation of a turbine driven gas generator or convective heat transfer script for the convection term for the combustion stage of a jet engine.

I utilized MATLAB’s built in syms functionality to numerically solve various systems of equations. However as this became increasingly computationally intensive, it became more efficient (and accurate) to manually define differential elements as functions of the other parameters either by hand or using equation solvers. Small note, scripts using syms will be _slighty_ less accurate (still very close and certainly good enough for first order approximation and system trade study level decisions). However, for best/most accurate results, use the "Combined_Positive_Shaft_Work_Skin_Friction_Convection.m" script and update any initial conditions manually to capture your specific use case. 
