...
I utilized MATLAB’s built in syms functionality to numerically solve various systems of equations. However as this became increasingly computationally intensive, it became more efficient (and accurate) to manually define differential elements as functions of the other parameters either by hand or using equation solvers, so some of the solutions to these one off case files made are _slighty_ less accurate. For best results, use the "Combined_Positive_Shaft_Work_Skin_Friction_Convection.m" script and update any initial conditions manually to capture your specific use case, as this accuratrely combines the complications of other one off cases into a single script.
