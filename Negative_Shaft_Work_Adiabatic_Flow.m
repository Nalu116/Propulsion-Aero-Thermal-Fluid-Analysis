...
%{
    Negative shaft work interaction to adiabatic flow
%}

n = 100000; %Axial step size
%n = 200; %Temporary Axial Step Size as a shield against primordials 

W = -1.8e5; %Total work per mass (J/kg)
dw = W/n;
...
syms M2q
eqn = mdot == Pt2*((y/R)^0.5)*(1/(Tt2)^0.5)*M2q*Ae*(1+(((y-1)*M2q^2)/2))^-((y+1)/(2*(y-1)));

M2 = double(vpasolve(eqn, M2q, [0 1]));
Te = Tt2/(1+(((y-1)/2)*M2^2));
...
percent = (i/n) * 100;
waitbar(percent/100,h,sprintf('%.2f%% of Negative Shaft Work Calculation Complete',percent))
...
fprintf('      NEGATIVE SHAFT WORK INTERACTIONS TO ADIABATIC FLOW  \n')
